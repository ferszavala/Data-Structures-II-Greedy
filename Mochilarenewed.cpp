...
int main() {
    int lenn = sizeof(obj_peso)/sizeof(obj_peso[0]); 
  
    for(int i=0;i<lenn;i++){
      obj_precio_peso[i][0] = double(obj_precio[i])/double(obj_peso[i]);
      obj_precio_peso[i][1] = double(obj_peso[i]);
    }
    for( int i = 0; i < lenn-1; i++ ){
        for( int j = i+1; j > 0 && obj_precio_peso[j][0] > obj_precio_peso[j-1][0]; j-- ){
          double temp[] = {obj_precio_peso[j][0], obj_precio_peso[j][1]};
